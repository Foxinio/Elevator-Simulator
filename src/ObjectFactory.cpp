...
static size_t maxLenght = 3;
static void alignTabs(std::string& string) {
	size_t len = string.length() / 8;
	maxLenght = maxLenght > len ? maxLenght : len;
	string += std::string(maxLenght - len + 1, '\t');
}

static std::mutex clogMutex{};
void ObjectFactory::PrintMessage(std::string sender, std::string message) {
	using namespace std::string_literals;
	std::string output = "["s + ObjectFactory::TimeToString(ObjectFactory::GetSimulationTime()) + "] " + sender + ":";
	alignTabs(output);
	output += message + "\n";
